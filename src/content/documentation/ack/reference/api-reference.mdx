---
title: API Reference
description: Browse a quick reference for Ack's core classes, methods, and annotations
---
...
Entry point for creating schemas. See [Schema Types](/documentation/ack/essentials/schemas).
...
- `Ack.codec(...)` / `Ack.date()` / `Ack.datetime()` / `Ack.uri()` /
  ...
  [Codecs](/documentation/ack/advanced/codecs).
...
Primitive schemas (`StringSchema`, `IntegerSchema`, `DoubleSchema`, `NumberSchema`, `BooleanSchema`) are strict — they reject values whose Dart runtime type doesn't match. `IntegerSchema` and `DoubleSchema` do not overlap (`42.0` fails `Ack.integer()`, `42` fails `Ack.double()`); use `Ack.number()` when either is acceptable. For non-`num` boundary types (e.g. numeric strings), use [`transform`](/documentation/ack/essentials/schemas#transformations) or [`codec`](#codecschemaboundary-runtime) to convert before validation.
...
See also [Schema Types](/documentation/ack/essentials/schemas) for detailed usage examples.

## `StringSchema`

Schema for validating strings. See [String Validation](/documentation/ack/essentials/validation#string-constraints).
...
- `matches(String pattern, {String? example, String? message})`: Must match a regex pattern. Patterns are not automatically anchored — use `^...$` for full-string matching. See [String validation](/documentation/ack/essentials/validation#string-constraints) for details.
...
values by default. See [Number Validation](/documentation/ack/essentials/validation#number-constraints).
...
Schema for validating arrays. See [List Validation](/documentation/ack/essentials/validation#list-constraints).
...
Schema for validating objects (maps). See [Object Validation](/documentation/ack/essentials/schemas#object).
...
Object returned by `safeParse()`. See [Error Handling](/documentation/ack/essentials/error-handling).
...
Represents a validation failure. See [Error handling](/documentation/ack/essentials/error-handling).
...
Base class for custom validation rules. See [Custom validation](/documentation/ack/how-to-guides/custom-validation).
...
See [Schema types](/documentation/ack/essentials/schemas) for the full catalogue,
...
[Model Code Generation](/documentation/ack/advanced/typesafe-schemas#schema-first-unions).
...
cases. See
[Model Code Generation](/documentation/ack/advanced/typesafe-schemas) for the complete
...
reverse direction. See [Codecs](/documentation/ack/advanced/codecs).
...
*See the [Schema types](/documentation/ack/essentials/schemas) guide for detailed usage and examples.*
